...
const dbInit = {
	...
	async v3_11DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN home_hero_desc TEXT NOT NULL DEFAULT '';`).run();
		} catch (e) {
	...
	async v3_10DB(c) {
		try {
			await c.env.db.batch([
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN announce_title TEXT NOT NULL DEFAULT '';`),
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN announce_content TEXT NOT NULL DEFAULT '';`),
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN announce_type TEXT NOT NULL DEFAULT 'none';`),
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN announce_version INTEGER NOT NULL DEFAULT 0;`)
			]);
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
		...
		try {
			await c.env.db.prepare(`
				UPDATE setting SET
					announce_title = CASE WHEN announce_title = '' THEN notice_title ELSE announce_title END,
					announce_content = CASE WHEN announce_content = '' THEN notice_content ELSE announce_content END
				WHERE announce_content = '' AND (notice_content != '' OR notice_title != '')
			`).run();
		} catch (e) {
			console.warn(`公告迁移：${e.message}`);
		}
	},

	async v3_9DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN notice_version INTEGER NOT NULL DEFAULT 0;`).run();
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	},
	...
	async v3_8DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN ui_motion INTEGER NOT NULL DEFAULT 0;`).run();
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	},
	...
	async v3_7DB(c) {
		try {
			await c.env.db.batch([
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN forward_error TEXT NOT NULL DEFAULT '';`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN forward_error_time TEXT NOT NULL DEFAULT '';`)
			]);
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	...
	async v3_6DB(c) {
		try {
			...
			`).run();
			await c.env.db.prepare(`CREATE INDEX IF NOT EXISTS idx_site_message_to_read ON site_message(to_user_id, is_read, message_id)`).run();
			await c.env.db.prepare(`CREATE INDEX IF NOT EXISTS idx_site_message_pair ON site_message(from_user_id, to_user_id, message_id)`).run();
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	},
	...
	async v3_5DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE user ADD COLUMN forward_apply INTEGER NOT NULL DEFAULT 0;`).run();
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	},
	...
	async v3_4DB(c) {
		try {
			await c.env.db.batch([
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN forward_email TEXT NOT NULL DEFAULT '';`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN forward_status INTEGER NOT NULL DEFAULT 1;`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN tg_chat_id TEXT NOT NULL DEFAULT '';`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN tg_bot_status INTEGER NOT NULL DEFAULT 1;`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN webhook_url TEXT NOT NULL DEFAULT '';`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN webhook_status INTEGER NOT NULL DEFAULT 1;`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN webhook_retry INTEGER NOT NULL DEFAULT 0;`),
				c.env.db.prepare(`ALTER TABLE user ADD COLUMN webhook_secret TEXT NOT NULL DEFAULT '';`)
			]);
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
