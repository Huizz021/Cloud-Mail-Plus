...
const telegramService = {
	...
	async sendEmailToBot(c, email, chatIds) {

		const { tgBotToken, customDomain, tgMsgTo, tgMsgFrom, tgMsgText } = await settingService.query(c);

		if (!tgBotToken || !chatIds?.length) {
			return;
		}

		const tgChatIds = Array.isArray(chatIds) ? chatIds : String(chatIds).split(',').map(id => id.trim()).filter(Boolean);
