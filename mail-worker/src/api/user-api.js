...
app.get('/user/forwardList', async (c) => {
	const data = await userService.listForward(c);
	return c.json(result.ok(data));
});

app.get('/user/forwardPendingCount', async (c) => {
	const data = await userService.listForward(c);
	const pending = (data.list || []).filter(item => item.forwardApply === 1).length;
	return c.json(result.ok({ pending }));
});

app.put('/user/forwardReview', async (c) => {
	const data = await userService.reviewForward(c, await c.req.json());
	return c.json(result.ok(data));
});
