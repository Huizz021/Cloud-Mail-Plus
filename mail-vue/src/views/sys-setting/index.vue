  ...
  <div class="settings-container">
    ...
    <el-scrollbar class="scroll" v-if="!firstLoading">
      <div class="scroll-body">
        ...
        <div class="card-grid">
          <!-- Basic Settings Card -->
          <div id="set-basic" v-show="settingSectionVisible('set-basic')" class="settings-card">
          ...
          <div id="set-look" v-show="settingSectionVisible('set-look')" class="settings-card">
          ...
          <div id="set-mail" v-show="settingSectionVisible('set-mail')" class="settings-card">
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                <div>
                  <span>{{ $t('uiMotion') }}</span>
                  <el-tooltip effect="dark" :content="$t('uiMotionDesc')">
                    <Icon class="warning" icon="fe:warning" width="18" height="18"/>
                  </el-tooltip>
                </div>
                <div>
                  <el-select
                      :style="`width: ${ locale === 'en' ? 130 : 110 }px;`"
                      v-model="setting.uiMotion"
                      placeholder="Select"
                      @change="onUiMotionChange"
                  >
                    <el-option :label="$t('uiMotionClassic')" :value="0"/>
                    <el-option :label="$t('uiMotionIos')" :value="1"/>
                  </el-select>
                </div>
              </div>
              <div class="setting-item">
          ...
          <div id="set-push" v-show="settingSectionVisible('set-push')" class="settings-card">
            ...
            <div class="card-content">
              ...
              <div class="setting-item">
                <div>
                  <span>{{ $t('adminMonitorEmail') }}</span>
                  <el-tooltip effect="dark" :content="$t('adminMonitorEmailDesc')">
              ...
              <div class="setting-item">
                <div>
                  ...
                  <el-tooltip effect="dark" :content="$t('forwardApplyDesc')">
                    <Icon class="warning" icon="fe:warning" width="16" height="16"/>
                  </el-tooltip>
                </div>
                <div class="forward">
                  <span>{{ uiStore.forwardReviewPending > 0 ? t('forwardApplyPending') + ' (' + uiStore.forwardReviewPending + ')' : $t('forwardApply') }}</span>
                  <el-button class="opt-button" size="small" type="primary" @click="openForwardReview">
                    <Icon icon="fluent:settings-48-regular" width="18" height="18"/>
                  </el-button>
                </div>
              </div>
          ...
          <!-- Object Storage Card -->
          <div id="set-oss" v-show="settingSectionVisible('set-oss')" class="settings-card">
          ...
          <div id="set-cf" v-show="settingSectionVisible('set-cf')" class="settings-card">
          ...
          <div id="set-home" v-show="settingSectionVisible('set-notice-doc')" class="settings-card">
            <div class="card-title">{{ $t('homeHeroDescTitle') }}</div>
            <div class="card-content">
              <div class="announce-editor">
                <div class="announce-hint">{{ $t('homeHeroDescHint') }}</div>
                <div class="announce-field">
                  <label>{{ $t('homeHeroDescTitle') }}</label>
                  <el-input
                      v-model="homeHeroDescInput"
                      type="textarea"
                      :autosize="{ minRows: 3, maxRows: 8 }"
                      maxlength="500"
                      show-word-limit
                      :placeholder="t('homeHeroDescHint')"
                  />
                </div>
                <div class="announce-actions">
                  <el-button type="primary" :loading="settingLoading" @click="saveHomeHeroDesc">
                    {{ $t('save') }}
                  </el-button>
                </div>
              </div>
          ...
          <div id="set-announce" v-show="settingSectionVisible('set-notice-doc')" class="settings-card">
            <div class="card-title">{{ $t('siteAnnouncement') }}</div>
            <div class="card-content">
              <div class="announce-editor">
                <div class="announce-hint">{{ $t('announceOnlyHint') }}</div>
                <div class="announce-field">
                  <label>{{ $t('titleDesc') }}</label>
                  <el-input v-model="announceForm.announceTitle" :placeholder="t('titleDesc')" maxlength="80" show-word-limit/>
                </div>
                <div class="announce-field">
                  <label>{{ $t('noticeContentDesc') }}</label>
                  <el-input
                      v-model="announceForm.announceContent"
                      type="textarea"
                ...
                <div class="announce-actions">
                  <el-button type="primary" :loading="settingLoading" @click="publishAnnouncementNow">
                    {{ $t('publishAnnouncement') }}
                  </el-button>
                  <el-button @click="previewAnnouncement">{{ $t('popUp') }}</el-button>
                </div>
              </div>
            </div>
          </div>

          <div id="set-login-notice" v-show="settingSectionVisible('set-notice-doc')" class="settings-card">
            <div class="card-title">{{ $t('noticePopup') }}</div>
            <div class="card-content">
              <div class="announce-editor">
                <div class="announce-hint">{{ $t('loginNoticeOnlyHint') }}</div>
                <div class="announce-field">
                  <label>{{ $t('titleDesc') }}</label>
                  <el-input v-model="noticeForm.noticeTitle" :placeholder="t('titleDesc')" maxlength="80" show-word-limit/>
                </div>
                <div class="announce-field">
                  <label>{{ $t('noticeContentDesc') }}</label>
                  <el-input
                ...
                <div class="announce-actions">
                  <el-button type="primary" :loading="settingLoading" @click="saveNoticePopup">
                    {{ $t('save') }}
                  </el-button>
                  <el-button @click="previewNoticePopup">{{ $t('popUp') }}</el-button>
                  <el-button @click="openNoticePopupSetting">{{ $t('advanced') }}</el-button>
                  <div class="notice-inline-switch">
                    <span>{{ setting.notice === 0 ? $t('enabled') : $t('disabled') }}</span>
                    <el-switch v-model="noticeForm.notice" :active-value="0" :inactive-value="1"
                               :active-text="$t('enable')" :inactive-text="$t('disable')"
                               @change="saveNoticePopup"/>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div id="set-ai" v-show="settingSectionVisible('set-ai')" class="settings-card">
          ...
          <!-- OAuth Login Card -->
          <div id="set-oauth" v-show="settingSectionVisible('set-oauth')" class="settings-card">
          ...
          <div id="set-about" v-show="settingSectionVisible('set-about')" class="settings-card about wide">
      ...
      >
        <template #header>
          <div class="forward-head">
            <span class="forward-set-title">{{ $t('adminMonitorEmail') }}</span>
            <el-tooltip effect="dark" :content="$t('adminMonitorEmailDesc')">
              <Icon class="warning" icon="fe:warning" width="18" height="18"/>
            </el-tooltip>
          </div>
...
Authorization: &lt;secret&gt;</pre>
      ...
      <el-dialog class="forward-review-dialog" v-model="forwardReviewShow" :title="$t('forwardApply')"
                 width="860px" top="6vh" @open="loadForwardReview">
        <div class="forward-review-desc">{{ $t('forwardApplyDesc') }}</div>
        <div v-if="!forwardCfConfigured" class="forward-review-desc cf-warn">{{ $t('cfConfigHint') }}</div>
        <div v-else-if="forwardCfError" class="forward-review-desc cf-warn">CF API: {{ forwardCfError }}</div>
        <el-table :data="forwardReviewList" v-loading="forwardReviewLoading" class="forward-review-table"
                  max-height="420">
          <el-table-column property="email" :label="$t('forwardReviewUser')" min-width="160" show-overflow-tooltip/>
          <el-table-column property="forwardEmail" :label="$t('forwardReviewEmail')" min-width="180" show-overflow-tooltip/>
          <el-table-column :label="$t('cfStatus')" min-width="120">
            <template #default="{ row }">
              <div class="cf-tags">
                <template v-if="row.cfStatus && row.cfStatus.length">
                  <el-tag
                      v-for="cf in row.cfStatus"
                      :key="cf.address"
                      size="small"
                      effect="plain"
                      :type="cfTagType(cf.status)"
                  >
                    {{ cf.status === 'verified' ? $t('cfVerified') : cf.status === 'pending' ? $t('cfPending') : cf.status === 'unconfigured' ? $t('cfUnconfigured') : $t('cfUnknown') }}
                  </el-tag>
                </template>
                <span v-else>-</span>
              </div>
          ...
          <el-table-column :label="$t('forwardReviewStatus')" width="100" align="center">
            <template #default="{ row }">
              <el-tag :type="forwardApplyTagType(row.forwardApply)" size="small" effect="plain">
                {{ forwardApplyText(row.forwardApply) }}
              </el-tag>
              <div v-if="row.forwardError" class="forward-error-tip" :title="row.forwardError">
                {{ $t('forwardError') }}
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('forwardReviewActive')" width="90" align="center">
            <template #default="{ row }">
              <el-tag :type="row.forwardActive ? 'success' : 'info'" size="small" effect="plain">
                {{ row.forwardActive ? $t('forwardApplyActive') : $t('forwardApplyInactive') }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column :label="$t('forwardReviewAction')" width="150" align="center" fixed="right">
            <template #default="{ row }">
              <div class="forward-review-actions">
                <el-button size="small" type="success" :disabled="!row.forwardEmail || row.forwardApply === 2"
                           :loading="forwardReviewLoading" @click="reviewForward(row, true)">
                  {{ $t('forwardApprove') }}
                </el-button>
                <el-button size="small" type="danger" :disabled="!row.forwardEmail || row.forwardApply === 3"
                           :loading="forwardReviewLoading" @click="reviewForward(row, false)">
                  {{ $t('forwardReject') }}
                </el-button>
              </div>
            </template>
      ...
      <el-dialog top="5vh" v-model="noticePopupShow" :title="$t('siteAnnouncement')" class="notice-popup"
        ...
        <form @submit.prevent>
          ...
          <div style="margin-bottom: 8px;color: var(--regular-text-color);font-size: 12px;">
            {{ $t('noticePublishDesc') }}
          </div>
            <el-input
                v-model="noticeForm.noticeContent"
...
import {deleteBackground, setBackground, setBlackList, settingQuery, settingSet, publishNotice} from "@/request/setting.js";
import {userForwardList, userForwardReview} from "@/request/user.js";
...
import {applyUiMotion} from "@/utils/ui-motion.js";
...
const forwardReviewShow = ref(false)
const forwardReviewLoading = ref(false)
const forwardReviewList = ref([])
const forwardCfConfigured = ref(false)
const forwardCfError = ref('')

function forwardApplyText(apply) {
  const map = {
  ...
  return map[apply] || t('forwardApplyNone')
}

function forwardApplyTagType(apply) {
  if (apply === 1) return 'warning'
  if (apply === 2) return 'success'
  if (apply === 3) return 'danger'
  return 'info'
}

function cfTagType(status) {
  if (status === 'verified') return 'success'
  if (status === 'pending') return 'warning'
  return 'info'
}

function openForwardReview() {
  forwardReviewShow.value = true
  loadForwardReview()
}

function loadForwardReview() {
  forwardReviewLoading.value = true
  userForwardList().then(data => {
    if (Array.isArray(data)) {
      forwardReviewList.value = data
      forwardCfConfigured.value = false
    } else {
      forwardReviewList.value = Array.isArray(data?.list) ? data.list : []
      forwardCfConfigured.value = !!data?.cfConfigured
      forwardCfError.value = data?.cfError || ''
    }
    uiStore.forwardReviewPending = forwardReviewList.value.filter(item => item.forwardApply === 1).length
  }).finally(() => {
    forwardReviewLoading.value = false
  })
}

function reviewForward(row, approved) {
  if (forwardReviewLoading.value) return
  forwardReviewLoading.value = true
  userForwardReview({ userId: row.userId, approved }).then(() => {
    ElMessage({
      message: approved ? t('forwardReviewOk') : t('forwardRejectOk'),
...
watch(() => uiStore.openForwardReviewAt, (val) => {
  if (val > 0) {
    openForwardReview()
  }
})
...
const homeHeroDescInput = ref('')
const activeNav = ref('set-basic')

const settingNav = [
  { id: 'set-basic', title: 'websiteSetting' },
  { id: 'set-look', title: 'customization' },
  { id: 'set-mail', title: 'emailSetting' },
  { id: 'set-push', title: 'emailPush' },
  { id: 'set-oss', title: 'oss' },
  { id: 'set-cf', title: 'turnstileSetting' },
  { id: 'set-notice-doc', title: 'noticeDocSection' },
  { id: 'set-ai', title: 'AI' },
  { id: 'set-oauth', title: 'oauthLogin' },
  { id: 'set-about', title: 'about' }
]
...
function settingSectionVisible(id) {
...
function getSettings() {
  ...
  settingQuery().then(settingData => {
    ...
    if (settingData.uiMotion !== undefined) {
      applyUiMotion(settingData.uiMotion)
      if (settingStore.settings) settingStore.settings.uiMotion = settingData.uiMotion
    }
    // 鐧诲綍寮圭獥鍐呭锛坣otice_*锛?
    noticeForm.notice = settingData.notice
    noticeForm.noticeTitle = settingData.noticeTitle || ''
    noticeForm.noticeContent = settingData.noticeContent || ''
    noticeForm.noticeType = settingData.noticeType || 'none'
    noticeForm.noticeDuration = settingData.noticeDuration || 0
    noticeForm.noticePosition = settingData.noticePosition || 'top-right'
    noticeForm.noticeOffset = settingData.noticeOffset || 0
    noticeForm.noticeWidth = settingData.noticeWidth || 400
    // 鍏ㄧ珯鍏憡鍐呭锛坅nnounce_*锛屼笌鐧诲綍寮圭獥鍒嗙锛?
    announceForm.announceTitle = settingData.announceTitle || settingData.noticeTitle || ''
    announceForm.announceContent = settingData.announceContent || ''
    homeHeroDescInput.value = settingData.homeHeroDesc || ''
...
function saveHomeHeroDesc() {
  if (settingLoading.value) return
  const value = String(homeHeroDescInput.value || '')
  settingLoading.value = true
  settingSet({ homeHeroDesc: value }).then(() => {
    setting.value.homeHeroDesc = value
    if (settingStore.settings) settingStore.settings.homeHeroDesc = value
...
function publishAnnouncementNow() {
  if (settingLoading.value) return
  const title = String(announceForm.announceTitle || '').trim()
  const content = String(announceForm.announceContent || '').trim()
  if (!title && !content) {
    ElMessage({ message: t('announcementEmpty'), type: 'warning', plain: true })
    return
  }
  settingLoading.value = true
  const payload = {
    announceTitle: title || '鍏憡',
    announceContent: content,
    announceType: noticeForm.noticeType || setting.value.announceType || 'none',
    noticeDuration: noticeForm.noticeDuration || 0,
    noticePosition: noticeForm.noticePosition || setting.value.noticePosition || 'top-right',
    noticeOffset: noticeForm.noticeOffset || 0,
    noticeWidth: noticeForm.noticeWidth || 400
  }
  publishNotice(payload).then(data => {
    const count = data?.messageSent ?? data?.userCount ?? 0
    ElMessage({
      message: t('announcementSent', { count }),
      type: 'success',
      plain: true
    })
    const merged = {
      announceTitle: payload.announceTitle,
      announceContent: payload.announceContent,
      announceType: payload.announceType,
      announceVersion: data?.announceVersion ?? data?.noticeVersion
    }
    setting.value = { ...setting.value, ...merged }
    if (settingStore.settings) {
      settingStore.settings = { ...settingStore.settings, ...merged }
    }
    announceForm.announceTitle = payload.announceTitle
    announceForm.announceContent = payload.announceContent
  }).catch(() => {}).finally(() => {
    settingLoading.value = false
  })
}
...
function previewAnnouncement() {
  uiStore.previewNotice({
    notice: 0,
    noticeWidth: noticeForm.noticeWidth || 400,
    noticeTitle: announceForm.announceTitle || setting.value.announceTitle || '',
    noticeContent: announceForm.announceContent || setting.value.announceContent || '',
    noticeType: noticeForm.noticeType || setting.value.announceType || 'none',
    noticeDuration: noticeForm.noticeDuration || 0,
    noticePosition: noticeForm.noticePosition || 'top-right',
    noticeOffset: noticeForm.noticeOffset || 0
  })
}
...
function onUiMotionChange(value) {
  if (!settingReady.value) return
  setting.value.uiMotion = value
  applyUiMotion(value)
  if (settingStore.settings) settingStore.settings.uiMotion = value
  editSetting({ uiMotion: value }, false)
}

// 鏆楄壊鍒囨崲鍚庝笉瑕佹竻鎺?ui-motion 绫伙紙宸茬敱 applyDarkMode 澶勭悊锛?
...
function editSetting(settingForm, refreshStatus = true) {
  ...
  settingSet(settingForm).then(() => {
    settingLoading.value = false
    if (settingForm && settingForm.uiMotion !== undefined) {
      applyUiMotion(settingForm.uiMotion)
      if (settingStore.settings) settingStore.settings.uiMotion = settingForm.uiMotion
    }
    ElMessage({
      message: t('saveSuccessMsg'),
      type: "success",
...
:deep(.forward-review-dialog.el-dialog) {
  width: min(860px, calc(100vw - 48px)) !important;
  max-width: calc(100vw - 48px);
}

:deep(.forward-review-dialog .el-dialog__body) {
  padding-top: 8px;
  padding-bottom: 16px;
}
